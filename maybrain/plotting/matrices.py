...
def plot_avg_matrix(brains, size_brain=500, output_file=None, dummy_adj_file=rr.DUMMY_ADJ_FILE_500,
    ...
    size_brain: the number of nodes of the `adjMat`s
    ...
    for brn in brains.values():
        if brn.adjMat.shape != (size_brain, size_brain):
            raise TypeError("There is a brain which doesn't have the size defined in size_brain argument")
    ...
    fig, ax = plt.subplots(figsize=(size_brain / 50, size_brain / 50))
    # Plotting
    ...
    plt.colorbar(im)
    ax.set_title("Average Connection Strength Matrix", y=1.08)
