...
def degreewrite(brain,outfilebase="brain"):
    # node degrees
    outfile = outfilebase+'_degrees_nodes'

    try:
        if brain.iter == None and os.path.exists(outfile):
    ...
    if brain.iter == None and os.path.exists(outfileHist):
        print "Moving existing file to "+outfileHist+'.old'
        os.rename(outfileHist,outfileHist+'.old')
    
    if not os.path.exists(outfileHist):
...
def smallworldparameters(brain,outfilebase = "brain"):
...
def betweennesscentralitywrite(brain,outfilebase = "brain"):
...
def closenesscentralitywrite(brain,outfilebase = "brain"):
    """ Calculates node and hub betwenness centralities. For hub centralities there are two files, one with the values in and another
    with the hub identities in corresponding rows.
    """
...
def efficiencywrite(brain,outfilebase = "brain"):
    """
...
def modularstructure(brain,outfilebase = "brain", redefine_clusters=True):
    """
...
def writeEdgeLengths(brain,outfilebase = "brain"):
